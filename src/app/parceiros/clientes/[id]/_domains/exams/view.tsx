...
const inputClass = "box-border h-10 w-full min-w-0 max-w-full rounded-[8px] border border-[#303746] bg-[#081520] px-3 text-[13px] text-white outline-none transition placeholder:text-[#667684] focus:border-[#3b97e3]";
...
const toneClasses: Record<PartnerClientExamResult["statusTone"], string> = {
  blue: "border-[#3b97e3] bg-[#102333] text-[#8fcfff]",
  green: "border-[#22c55e] bg-[#102d21] text-[#67dc90]",
  orange: "border-[#f97316] bg-[#321b0f] text-[#fb923c]",
  red: "border-[#ef4444] bg-[#34141b] text-[#ff7b88]",
  yellow: "border-[#eab308] bg-[#2c2614] text-[#f5d34d]",
};

const toneTextClasses: Record<PartnerClientExamResult["statusTone"], string> = {
  blue: "text-[#8fcfff]",
  green: "text-[#67dc90]",
  orange: "text-[#fb923c]",
  red: "text-[#ff7b88]",
...
const chartToneColors: Record<PartnerClientExamResult["statusTone"], string> = {
  blue: "#79c7ff",
  green: "#22c55e",
  orange: "#f97316",
  red: "#ef4444",
...
function Field({ children, label }: { children: ReactNode; label: string }) {
  return (
    <label className="grid min-w-0 gap-1.5 text-[11px] font-semibold uppercase tracking-[0.05em] text-[#8b92a3]">
      {label}
      {children}
    </label>
  );
...
function StatusBadge({ label, tone }: { label: string; tone: PartnerClientExamResult["statusTone"] }) {
  return <span className={cn("rounded-full border px-2.5 py-1 text-[11px] font-semibold", toneClasses[tone])}>{label}</span>;
}

type ExamChartTooltipProps = {
  active?: boolean;
  payload?: Array<{ payload?: unknown }>;
};

export function ExamChartTooltip({ active, payload }: ExamChartTooltipProps) {
  const point = payload?.[0]?.payload as PartnerClientExamResult | undefined;
  if (!active || !point) return null;

  return (
    <div className="min-w-[190px] rounded-[10px] border border-[#2f82bf] bg-[#071827] p-3 text-white shadow-[0_12px_30px_rgba(0,0,0,0.35)]">
      <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#8b92a3]">Data da coleta</p>
      <p className="mt-1 text-[12px] font-semibold text-white">{point.collectedLabel}</p>
      <div className="my-3 border-t border-[#263846]" />
      <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#8b92a3]">Valor atual</p>
      <p className="mt-1 text-[20px] font-bold leading-6 text-white">{point.valueLabel}</p>
      <p className="mt-3 text-[12px] text-[#b8c4ce]">Referência: {point.referenceLabel}</p>
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <span className={cn("rounded-full border px-2 py-1 text-[10px] font-bold", toneClasses[point.deviationTone])}>{point.statusLabel}</span>
        <span className="text-[11px] font-semibold text-[#d8e5ee]">{point.deviationLabel}</span>
      </div>
      {point.deviationPct !== null && point.deviationLevel !== "normal" ? (
        <p className="mt-2 text-[11px] text-[#8b92a3]">Desvio de {formatExamNumber(point.deviationPct, 1)}%</p>
      ) : null}
    </div>
  );
}

function ExamChartDot({ cx, cy, payload }: { cx?: number; cy?: number; payload?: PartnerClientExamResult }) {
  if (typeof cx !== "number" || typeof cy !== "number") return null;
  return <circle cx={cx} cy={cy} fill={chartToneColors[payload?.statusTone ?? "blue"]} r="4.5" stroke="#f3f4f7" strokeWidth="1.5" />;
}

function ExamHistoryChart({ results }: { results: PartnerClientExamResult[] }) {
  ...
  useEffect(() => {
    ...
    const update = () => setChartWidth(Math.floor(element.getBoundingClientRect().width));
    update();
  ...
  if (points.length === 0) return <div className="flex h-[190px] items-center justify-center text-[12px] text-[#708597]">Sem histórico no período.</div>;

  const values = points.map((point) => point.valueDefault);
  const latestPoint = points[points.length - 1];
  const referenceValues = [latestPoint.referenceLow, latestPoint.referenceHigh].filter((value): value is number => value !== null);
  const referenceThresholdValues = [
    latestPoint.referenceLow !== null && latestPoint.referenceLow > 0 ? latestPoint.referenceLow * 0.75 : null,
    latestPoint.referenceHigh !== null && latestPoint.referenceHigh > 0 ? latestPoint.referenceHigh * 1.25 : null,
  ].filter((value): value is number => value !== null);
  const minValue = Math.min(...values, ...referenceValues, ...referenceThresholdValues, 0);
  const maxValue = Math.max(...values, ...referenceValues, ...referenceThresholdValues, 1);
  const range = Math.max(1, maxValue - minValue);
  const domain: [number, number] = [Math.max(0, minValue - range * 0.1), maxValue + range * 0.1];
  const high = latestPoint.referenceHigh;
  const low = latestPoint.referenceLow;

  return (
    <div aria-label={`Histórico de ${latestPoint.snapshotExamName} por data: ${points.map((point) => point.collectedLabel).join(", ")}`} className="h-[190px] min-w-0" data-testid="partner-client-exam-history-chart" ref={chartRef} role="img">
      {chartWidth > 0 ? <LineChart data={points} height={190} margin={{ bottom: 4, left: -18, right: 8, top: 12 }} width={Math.max(chartWidth, 1)}>
          <ReferenceArea fill="#ef4444" fillOpacity={0.08} ifOverflow="extendDomain" y1={high === 0 ? 0 : high === null ? domain[1] : high * 1.25} y2={domain[1]} />
          {high !== null && high > 0 ? <ReferenceArea fill="#f97316" fillOpacity={0.1} y1={high * 1.1} y2={high * 1.25} /> : null}
          {high !== null && high > 0 ? <ReferenceArea fill="#eab308" fillOpacity={0.1} y1={high} y2={high * 1.1} /> : null}
          {low !== null && low > 0 ? <ReferenceArea fill="#ef4444" fillOpacity={0.08} y1={domain[0]} y2={low * 0.75} /> : null}
          {low !== null && low > 0 ? <ReferenceArea fill="#f97316" fillOpacity={0.1} y1={low * 0.75} y2={low * 0.9} /> : null}
          {low !== null && low > 0 ? <ReferenceArea fill="#eab308" fillOpacity={0.1} y1={low * 0.9} y2={low} /> : null}
          {low !== null && high !== null ? <ReferenceArea fill="#22c55e" fillOpacity={0.1} y1={low} y2={high} /> : null}
          <CartesianGrid stroke="#214058" strokeDasharray="4 7" vertical={false} />
          <XAxis
            axisLine={false}
          ...
          <YAxis
            ...
            domain={domain}
            tick={{ fill: "#8b92a3", fontFamily: "Rethink Sans", fontSize: 10 }}
            tickFormatter={(value) => formatExamNumber(Number(value), 0)}
            tickLine={false}
            width={42}
          />
          <Tooltip content={<ExamChartTooltip />} cursor={{ stroke: "#79c7ff", strokeDasharray: "4 4" }} />
          <Line activeDot={{ r: 6, stroke: "#fff", strokeWidth: 1.5 }} dataKey="valueDefault" dot={(props) => <ExamChartDot {...props} />} isAnimationActive={false} name="Valor" stroke="#2d9cff" strokeWidth={2.5} type="monotone" />
        </LineChart> : null}
    </div>
  );
}
                ...
                {category.results.slice(0, 7).map((result) => (
                  <article className="min-h-[168px] rounded-[8px] border border-[#263846] bg-[#0b1823] p-3 sm:min-h-[238px] sm:p-4" key={result.id}>
                    ...
                    <div className="mt-2 flex flex-wrap items-center justify-between gap-2 sm:gap-3">
                      <StatusBadge label={result.statusLabel} tone={result.statusTone} />
                    ...
                    <div className="mt-5"><ExamHistoryChart results={allResultsByExam.get(result.examId) ?? [result]} /></div>
                  </article>
                ))}
      ...
      <form className="grid gap-6" onSubmit={onSubmit}>
        ...
        <section className="grid gap-4">
          {visibleDefinitions.map((definition) => {
            ...
            const assessment = preview ? assessExamValue(preview.valueDefault, reference) : null;
            return (
              <div className={cn(panelClass, "grid min-w-0 gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,120px)_minmax(0,150px)_minmax(0,1fr)] lg:items-center lg:gap-5")} key={definition.id}>
                <div className="min-w-0">
                ...
                <Field label="Valor">
                  <input className={cn(inputClass, assessment ? toneClasses[assessment.deviationTone] : null)} inputMode="decimal" placeholder="0" value={draft.value} onChange={(event) => onDraftChange(definition.id, { value: event.target.value })} />
                </Field>
                <Field label="Unidade">
                ...
                <div className="grid min-w-0 gap-1">
                  ...
                  {preview && assessment ? (
                    <>
                      <p className={cn("break-words text-[13px] font-semibold", toneTextClasses[assessment.deviationTone])}>{formatExamNumber(preview.valueDefault, 1)} {definition.defaultUnit}</p>
                      <div className="flex min-w-0 flex-wrap gap-1.5">
                        <span className={cn("w-fit rounded-full border px-2 py-1 text-[10px] font-bold", toneClasses[assessment.deviationTone])}>{assessment.statusLabel}</span>
                        <span className="self-center break-words text-[11px] font-semibold text-[#d8e5ee]">{assessment.deviationLabel}</span>
                      </div>
                      {assessment.deviationPct !== null && assessment.deviationLevel !== "normal" ? <p className="text-[11px] text-[#8b92a3]">Desvio de {formatExamNumber(assessment.deviationPct, 1)}%</p> : null}
                    </>
                  ) : "Aguardando valor"}
                </div>
              </div>
