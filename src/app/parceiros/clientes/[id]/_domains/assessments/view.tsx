...
const compositionChartColors = ["#3b97e3", "#a277ff", "#ff7b8e", "#58a067", "#f0c76a", "#62d0ff"];
const skinfoldChartColors = ["#3b97e3", "#ff7b8e", "#a277ff", "#58a067", "#f0c76a", "#62d0ff", "#d678ff", "#f48c58"];
...
function formatChartValue(value: unknown) {
  return typeof value === "number" ? value.toLocaleString("pt-BR", { maximumFractionDigits: 1 }) : "";
}
...
function ChartLegend({
  ...
  return (
    <div className="mb-2 flex flex-wrap gap-x-3 gap-y-1 px-1 text-[10px] font-semibold text-[#9aa5b6]">
...
function CompositionChart({
  ...
  const stackedValues = data.map((row) => visible.reduce((total, metric) => {
  ...
  const domain = referenceProfile
    ...
    : mode === "stack"
      ? buildDynamicNumberDomain(stackedValues, 0.14)
  ...
  return (
    <div className="min-w-0 overflow-visible" data-testid="client-assessments-composition-chart" ref={ref}>
      {width > 0 && data.length > 0 && visible.length > 0 ? (
        <>
        <ChartLegend colors={compositionChartColors} metrics={visible} />
        <BarChart
          barCategoryGap={compact ? "18%" : "26%"}
          barGap={compact ? 2 : 5}
          data={data}
          height={260}
          margin={{ bottom: 4, left: -10, right: compact ? 12 : 24, top: 12 }}
          width={width}
        >
          ...
          <CartesianGrid stroke="#31536b" strokeDasharray="4 6" strokeOpacity={0.7} vertical={false} />
          <XAxis axisLine={false} dataKey="date" tick={{ fill: "#9aa5b6", fontSize: 11 }} tickLine={false} />
          <YAxis axisLine={false} domain={domain} tick={{ fill: "#9aa5b6", fontSize: 11 }} tickFormatter={(value) => formatChartValue(value)} tickLine={false} width={42} />
          <Tooltip
            wrapperStyle={{ maxWidth: Math.max(180, width - 24), zIndex: 20 }}
            contentStyle={{ background: "#071827", border: "1px solid #2f82bf", borderRadius: 8, color: "#f4f8fb" }}
          ...
          {visible.map((metric, index) => (
            <Bar
              dataKey={metric.key}
              fill={compositionChartColors[index % compositionChartColors.length]}
              key={metric.key}
              name={metric.label}
              radius={mode === "stack" ? index === visible.length - 1 ? [4, 4, 0, 0] : 0 : [4, 4, 0, 0]}
              stackId={mode === "stack" ? "composition" : undefined}
            >
              <LabelList dataKey={metric.key} fill="#f4f8fb" fontSize={compact ? 9 : 10} formatter={formatChartValue} position="inside" />
            </Bar>
          ))}
        </BarChart>
        {referenceProfile ? (
...
function SkinfoldChart({
  ...
  const { ref, width } = useMeasuredWidth();
  const visibleMetrics = metrics.filter((metric) => selectedMetrics.includes(metric.key));
  const stackedValues = data.map((row) => visibleMetrics.reduce((total, metric) => {
    const value = row[metric.key];
    return total + (typeof value === "number" ? value : 0);
  }, 0));
  const domain = buildDynamicNumberDomain(mode === "general" ? stackedValues : visibleMetrics.flatMap((metric) => data.map((row) => row[metric.key] as number | null)), 0.16);
  const latest = data.at(-1);
  const radarData = visibleMetrics.map((metric) => ({
    metric: metric.label,
    value: typeof latest?.[metric.key] === "number" ? latest[metric.key] as number : 0,
  ...
  return (
    <div className="min-w-0 overflow-visible" data-testid="client-assessments-skinfold-chart" ref={ref}>
      {width > 0 && data.length > 0 && visibleMetrics.length > 0 ? (
        mode === "radar" ? (
          <RadarChart cx="50%" cy="50%" data={radarData} height={260} outerRadius={Math.min(82, Math.max(54, width / 5))} width={width}>
            <PolarGrid stroke="#31536b" />
          ...
          <>
          <ChartLegend colors={skinfoldChartColors} metrics={visibleMetrics} />
          <BarChart barCategoryGap={width < 520 ? "18%" : "26%"} barGap={width < 520 ? 2 : 5} data={data} height={260} margin={{ bottom: 4, left: -10, right: 6, top: 12 }} width={width}>
            <CartesianGrid stroke="#31536b" strokeDasharray="4 6" strokeOpacity={0.7} vertical={false} />
            <XAxis axisLine={false} dataKey="date" tick={{ fill: "#9aa5b6", fontSize: 11 }} tickLine={false} />
            <YAxis axisLine={false} domain={domain} tick={{ fill: "#9aa5b6", fontSize: 11 }} tickFormatter={(value) => formatChartValue(value)} tickLine={false} width={42} />
            <Tooltip
            ...
            {visibleMetrics.map((metric, index) => (
              <Bar
                dataKey={metric.key}
                fill={skinfoldChartColors[index % skinfoldChartColors.length]}
                key={metric.key}
                name={metric.label}
                radius={index === visibleMetrics.length - 1 ? [4, 4, 0, 0] : 0}
                stackId="skinfold"
              >
                <LabelList dataKey={metric.key} fill="#f4f8fb" fontSize={width < 520 ? 9 : 10} formatter={formatChartValue} position="inside" />
              </Bar>
            ))}
          </BarChart>
          </>
...
export function PartnerClientAssessmentsView({ assessments, overview }: PartnerClientAssessmentsViewProps) {
  ...
  const [skinfoldMode, setSkinfoldMode] = useState<"general" | "radar">("general");
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            ...
            <Panel className="overflow-hidden p-0">
              <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[#303746] p-5">
                ...
                <ModeToggle modes={[{ label: "Geral", value: "general" }, { label: "Radar", value: "radar" }]} value={skinfoldMode} onChange={(value) => setSkinfoldMode(value as "general" | "radar")} />
              </div>
              <div className="grid lg:grid-cols-[260px_minmax(0,1fr)]">
                <MetricSelector
