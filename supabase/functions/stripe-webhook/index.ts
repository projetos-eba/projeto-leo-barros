...
function planSlugFromSubscriptionItems(items: unknown): BillingPlanSlug | null {
  ...
  for (const item of data) {
    ...
    if (priceId === OFFICIAL_STRIPE_PRICES["complete-annual"].id) {
      return "complete-annual";
    }
  }
...
function promotionFromDiscounts(subscription: Record<string, unknown>) {
  const discounts = Array.isArray(subscription.discounts)
    ...
    ? [subscription.discount]
    : [];
  const discount = asRecord(
    discounts.find((candidate) => Boolean(asRecord(candidate))),
  );
  ...
  return {
    ...
    label: describeCoupon(
      coupon as unknown as Parameters<typeof describeCoupon>[0],
    ),
    promotionCodeId: textValue(promotionCode?.id),
  };
}

function promotionHasDisplayData(
  promotion: ReturnType<typeof promotionFromDiscounts>,
) {
  return Boolean(
    promotion?.code || promotion?.couponId || promotion?.duration ||
      promotion?.label || promotion?.promotionCodeId,
  );
}
...
async function syncFinancialSummaryFromStripeSubscription({
  ...
  try {
    ...
    const quantity = activeClientQuantityFromSubscriptionItems(
    ...
    const { data: persistedSummary } = promotionHasDisplayData(stripePromotion)
      ...
      : await supabase
        ...
        .select(
          "discount_code, discount_duration, discount_label, stripe_coupon_id, stripe_promotion_code_id",
        )
        .eq("subscription_id", localSubscriptionId)
        .maybeSingle();
    const promotion = {
      code: stripePromotion?.code ?? persistedSummary?.discount_code ?? null,
      couponId: stripePromotion?.couponId ??
        persistedSummary?.stripe_coupon_id ?? null,
      duration: stripePromotion?.duration ??
        persistedSummary?.discount_duration ?? null,
      label: stripePromotion?.label ?? persistedSummary?.discount_label ?? null,
      promotionCodeId: stripePromotion?.promotionCodeId ??
        persistedSummary?.stripe_promotion_code_id ?? null,
    };
    ...
    if (
      ...
      if (partnerId) {
        ...
        const previousEventAt =
          currentSubscription?.stripe_last_event_created_at
            ? new Date(currentSubscription.stripe_last_event_created_at)
              .getTime()
            : 0;
        ...
        await supabase.from("partner_subscriptions").update({
          ...
          current_period_start: subscription.current_period_start
            ...
            : undefined,
          ended_at: event.type === "customer.subscription.deleted"
    ...
    if (event.type === "invoice.finalized") {
      ...
      if (localSubscription) {
        await supabase.from("billing_active_client_snapshots").insert({
          ...
          amount_cents: (localSubscription.active_client_quantity ?? 0) *
            ACTIVE_CLIENT_UNIT_CENTS,
    ...
    if (stripeEventId) {
      ...
      return jsonResponse(500, {
        error: {
          ...
          message: "Webhook recebido, mas nao processado.",
        },
      }, request);
    }
